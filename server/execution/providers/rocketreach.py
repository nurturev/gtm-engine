...
RocketReach API quirks handled here:
    ...
    - Lookup can return status="progress" (async) — need to poll check_status
...
def _prepare_enrich_person(params: dict[str, Any]) -> dict[str, Any]:
    ...
    # Universal person lookup gates contact data behind per-field reveal flags.
    # Pinned here so clients cannot override them. return_cached_emails is set
    # explicitly because the vendor-side default flips to false on 2026-05-01.
...
class RocketReachProvider(BaseProvider):
    ...
    async def execute(
        ...
        if response.status_code == 403:
            ...
            # credit allocation. Map this to 402 to match the insufficient-
            # credits convention used elsewhere in the server (see
            # execution/router.py :109, :358). A generic 403 stays 403.
            body_detail = ""
            try:
        ...
        async with httpx.AsyncClient() as client:
            while True:
                ...
                if response.status_code != 200:
                    # Transient / unexpected — try again on next tick.
                    continue
