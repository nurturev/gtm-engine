...
Create a Helm chart for the nrv-api service in the `helm-charts` repo, following the exact same pattern as `user-management-ws`.
...
- Secret naming: `{appName}-secret` → `nrv-api-secret`
- Ingress DNS: `{service}.public.{env}.nurturev.com` → `nrv-api.public.{env}.nurturev.com`
...
All files created in `/Users/nikhilojha/Projects/helm-charts/nrv-api/`:
...
- `appConf.appName: nrv-api`
- `image.repository: 979176640062.dkr.ecr.ap-south-1.amazonaws.com/nrv-api`
- `ingress.rules[0].host: nrv-api.public.staging.nurturev.com`
- `ports[0].target_port: 8000` (FastAPI, not 8080)
- Health checks on `/health` port 8000
- All secrets via `secretKeyRef` referencing `nrv-api-secret`
...
- Region: `us-east-1`
- ECR: `979176640062.dkr.ecr.us-east-1.amazonaws.com/nrv-api`
- Host: `nrv-api.public.prod.nurturev.com`
...
Same as staging with `NAMESPACE="prod"`, `VALUES_FILE="values-prod.yaml"`, `SECRET_FILE="nrv-api-secret-prod.yaml"`, and `prod-eks` cluster name.

---

## Key Differences from user-management-ws

| Setting | Org Standard (user-management-ws) | nrv-api | Why Different |
|---------|-------------------|---------|---------------|
...
| Secret name | `user-management-ws-secret` | `nrv-api-secret` | Follows `{appName}-secret` pattern |
| Ingress host | `umws.public.{env}.nurturev.com` | `nrv-api.public.{env}.nurturev.com` | Follows org DNS convention |
...
- [ ] `helm template nrv-api . -f values-staging.yaml` renders valid manifests
...
# Dry run
helm template nrv-api . -f values-staging.yaml > /tmp/nrv-api-manifests.yaml
# Review the output for correctness

# Check port mapping
grep -A2 "containerPort" /tmp/nrv-api-manifests.yaml
